...
class AdvancedAnalyticsService:
    ...
    async def generate_revenue_forecast(
        ...
        try:
            ...
            forecast_data = self._generate_forecast_data(forecast_days, historical_data)
    ...
    def _generate_forecast_data(
        ...
        Args:
            ...
            historical_data: Historical revenue data for calculating averages
            
        Returns:
            NumPy array of feature vectors for prediction
        """
        base_date = datetime.utcnow().date()
        forecast_features = []
        
        # Calculate historical averages from actual data
        if historical_data:
            revenues = [r["revenue"] for r in historical_data]
            transactions = [r["transactions"] for r in historical_data]
            
            avg_transactions = int(np.mean(transactions)) if transactions else 20
            week_avg = float(np.mean(revenues[-7:])) if len(revenues) >= 7 else float(np.mean(revenues))
            month_avg = float(np.mean(revenues[-30:])) if len(revenues) >= 30 else float(np.mean(revenues))
        else:
            # Fallback values only if no historical data (shouldn't happen)
            avg_transactions = 20
            week_avg = 1000.0
            month_avg = 1000.0
        ...
        for i in range(forecast_days):
            ...
            feature_vector = [
                ...
                avg_transactions,         # Use historical average transaction count
                week_avg,                 # Recent 7-day average revenue
                month_avg                 # Recent 30-day average revenue
            ]
